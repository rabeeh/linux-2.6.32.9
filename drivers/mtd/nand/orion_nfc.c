...
#if 0
void nand_writel(struct pxa3xx_nand_info *info, unsigned int off, unsigned int val)
{
	printk("NFC Write: %08x = %08x\n", ((unsigned int)info->mmio_base + (off)), val);
	__raw_writel(val, (info->mmio_base + off));
}

unsigned int nand_readl(struct pxa3xx_nand_info *info, unsigned int off)
{
	unsigned int reg = __raw_readl(info->mmio_base + off);
	printk("NFC Read:  %08x = %08x\n", (unsigned int)(info->mmio_base + off), reg);
	return reg;
}
#endif
...
static struct pxa3xx_nand_timing st_timing = {
	.tCH	= 5,
	.tCS	= 20,
	.tWH	= 10,
	.tWP	= 12,
			...
			uint16_t cmd, int column, int page_addr)
{
	...
	if (info->use_dma)
		if (f->page_size < 2048)
			...
			info->data_size = (info->use_bch) ? 2080 : 2112;
	else
		if (f->page_size < 2048)
			info->data_size = (info->use_ecc) ? 520 : 528;				
		else
			if (info->use_bch) info->data_size = 2080;
			else if (info->use_ecc) info->data_size = 2088;
			else info->data_size = 2112;
#endif
...
static void start_data_dma(struct pxa3xx_nand_info *info, int dir_out)
{
	...
	if (dir_out) {
		...
		desc->dtadr = (info->mmio_phys_base + NDDB);
		desc->dcmd |= DCMD_INCSRCADDR | DCMD_FLOWTRG | DCMD_OVERREAD;
	} else {
		desc->dtadr = info->data_buff_phys;
		desc->dsadr = (info->mmio_phys_base + NDDB);
		desc->dcmd |= DCMD_INCTRGADDR | DCMD_FLOWSRC | DCMD_OVERREAD;
	}
				    ...
				    struct pxa3xx_nand_flash *f)
{
	...
	ndcr |= NDCR_ND_ARB_EN; /* enable arbiter always for NAND */
	ndcr |= (f->col_addr_cycles == 2) ? NDCR_RA_START : 0;
	ndcr |= (f->page_per_block == 64) ? NDCR_PG_PER_BLK : 0;
	ndcr |= (f->page_size == 2048) ? NDCR_PAGE_SZ : 0;
	ndcr |= (f->flash_width == 16) ? NDCR_DWIDTH_M : 0;
	ndcr |= (pdata->nfc_width == 16) ? NDCR_DWIDTH_C : 0;
