...
static long display_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FBIO_SET_DISPLAY_MODE:
		...
		pr_debug("Reserved!! Not implement yet.\n");
		break;
...
dovefb_display_init(void)
{
	...
	pr_debug("lcd_regbase = 0x%08x\n", lcd_regbase);
	pr_debug("lcdseq = %d, 1^lcdseq = %d\n", lcdseq, 1^lcdseq);
	pr_debug("lcdseq = %d, 0^lcdseq = %d\n", lcdseq, 0^lcdseq);
#endif
	printk(KERN_WARNING "dovefb_display_init\n");

	/* register character. */
	if (register_chrdev(30, "display tools", &display_fops))
		pr_debug("unable to get major %d for fb devs\n", 30);
	...
	if (lcd_regbase) {
		...
		pr_debug("debug lcd0 reg 0x104 = 0x%08x\n", x);
		x = readl( lcd1_regbase + 0x108 );
		pr_debug("debug lcd0 reg 0x104 = 0x%08x\n", x);
		x = readl( dcon_regbase + 0x000 );
		pr_debug("debug dcon reg 0x000 = 0x%08x\n", x);
#endif
	}

	printk(KERN_INFO "dovefb_display driver init ok.\n");
	return 0;
}
...
dovefb_display_exit(void)
{
	...
	printk(KERN_INFO "dovefb_display driver unload OK.\n");
}
