...
static audio_input_format audio_format = SPDIF_FORMAT;
...
{
	printk (KERN_ERR"ERROR at %s - error code %d\n",__FUNCTION__, errcode);
}

static void setup_audio_video(void)
{
	printk (KERN_INFO"Got into %s\n",__FUNCTION__);
	/*
...
//	set_video_mode(AD9889_1080i, RGB, AD9889_0_255, _16x9);
	...
	if (I2S_FORMAT == audio_format)
	{
		printk("SET audio format is I2S\n");
		set_audio_format(I2S);
	}
	else
	{
		printk("SET audio format is SPDIF\n");	
...
int ad9889_int_status;
EXPORT_SYMBOL(ad9889_int_status);
...
int adi9889_thread (void *data)
{
	...
	while (1) {
		counter ++;
		try_to_freeze();
		ad9889_int_status = ad9889_interrupt_handler(setup_audio_video);
		msleep(2000);
	}
...
{
        int rc;
	struct adi9889_i2c_platform_data *pdata;
	...
	if(pdata)
		audio_format = pdata->audio_format;
	else	
		audio_format = DEFAULT_AUDIO_INPUT_FORMAT;

	printk(KERN_INFO"%s: audio input format is %s\n",__FUNCTION__ , audio_format==I2S_FORMAT?"I2S":"SPDIF");
	...
	if (!adi9889_thread_struct)
		printk ( "Error creating kernel thread in %s\n",__FUNCTION__);
...
MODULE_AUTHOR("gaggery@marvell.com");
