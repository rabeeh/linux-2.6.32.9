...
/** \mainpage BMA020 Acceleration Sensor API
 * Copyright (C) 2007 Bosch Sensortec GmbH
 *  \section intro_sec Introduction
 * BMA020 3-axis digital Accelerometer Programming Interface
 * The BMA020 API enables quick access to Bosch Sensortec's 3-Axis digital accelerometer.
 * The only mandatory steps are: 
 *
 * 1. linking the target application's communication functions to the API (\ref BMA020_WR_FUNC_PTR, \ref BMA020_RD_FUNC_PTR)
 *
 * 2. calling the bma020_init() routine, which initializes all necessary data structures for using all functions
 *
...
/* BMA020 Macro for read and write commincation */
...
/** Define the calling convention of YOUR bus communication routine.
	...
	The BMA020_WR_FUNC_PTR would equal:

    #define	BMA020_WR_FUNC_PTR char (* bus_write)(unsigned char, unsigned char, unsigned char *, unsigned char)
	
	Parameters can be mixed as needed refer to the \ref BMA020_BUS_WRITE_FUNC  macro.
...
//#define BMA020_WR_FUNC_PTR char (* bus_write)(unsigned char, unsigned char *, unsigned char)
...
/** link makro between API function calls and bus write function
	...
	define BMA020_BUS_WRITE_FUNC(dev_addr, reg_addr, reg_data, wr_len)\
           bus_write(dev_addr, reg_addr, reg_data, wr_len)

    This macro lets all API functions call YOUR communication routine in a way that equals your definition in the \ref BMA020_WR_FUNC_PTR definition.


	      
*/
#define BMA020_BUS_WRITE_FUNC(dev_addr, reg_addr, reg_data, wr_len)\
           bus_write(reg_addr, reg_addr, reg_data, wr_len)
...
/** Define the calling convention of YOUR bus communication routine.
	...
	The BMA020_RD_FUNC_PTR would equal:

    #define	BMA020_RD_FUNC_PTR char (* bus_read)(unsigned char, unsigned char, unsigned char *, unsigned char)
	
	Parameters can be mixed as needed refer to the \ref BMA020_BUS_READ_FUNC  macro.
...
/** link makro between API function calls and bus read function
	...
	define BMA020_BUS_READ_FUNC(dev_addr, reg_addr, reg_data, wr_len)\
           bus_read(dev_addr, reg_addr, reg_data, wr_len)

    This macro lets all API functions call YOUR communication routine in a way that equals your definition in the \ref BMA020_WR_FUNC_PTR definition.

	\note: this macro also includes the "MSB='1'" for reading BMA020 addresses. 
...
//#define acc_sensor_write_bytes		i2c_bma020_write_buf
//#define acc_sensor_read_bytes 		i2c_bma020_read_buf	
...
/** BMA020 image registers data structure
	\brief Register type that contains all BMA020 image registers from address 0x0b to 0x15
	This structure can hold the complete image data of BMA020
...
typedef struct {
	struct input_polled_dev *bma020_idev;

	struct cdev bma_cdev;
	int app_major;			//major node no.
...
/** Macro to convert floating point low-g-thresholds in G to 8-bit register values.<br>
  * Example: BMA020_LG_THRES_IN_G( 0.3, 2.0) generates the register value for 0.3G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define BMA020_LG_THRES_IN_G( gthres, range)			((int)((256 * gthres ) / range))

/** Macro to convert floating point high-g-thresholds in G to 8-bit register values.<br>
  * Example: BMA020_HG_THRES_IN_G( 1.4, 2.0) generates the register value for 1.4G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define BMA020_HG_THRES_IN_G(gthres, range)				((int)((256 * gthres ) / range))

/** Macro to convert floating point low-g-hysteresis in G to 3-bit register values.<br>
  * Example: BMA020_LG_HYST_THRES_IN_G( 0.2, 2.0) generates the register value for 0.2G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define BMA020_LG_HYST_IN_G( ghyst, range )				((int)((8 * ghyst) / range))

/** Macro to convert floating point high-g-hysteresis in G to 3-bit register values.<br>
  * Example: BMA020_HG_HYST_THRES_IN_G( 0.2, 2.0) generates the register value for 0.2G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define BMA020_HG_HYST_IN_G( ghyst, range )				((int)((8 * ghyst) / range))


/** Macro to convert floating point G-thresholds to 8-bit register values<br>
  * Example: BMA020_ANY_MOTION_THRES_IN_G( 1.2, 2.0) generates the register value for 1.2G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */

#define BMA020_ANY_MOTION_THRES_IN_G( gthres, range)		((int)((256 * gthres ) / range))
...
*/
static int __devinit bma020_i2c_probe(struct i2c_client *client, const struct i2c_device_id *);
//static int __devinit bma020_probe(struct spi_device *spi);
static int __devexit bma020_i2c_remove(struct i2c_client*);
//static int __devexit bma020_remove(struct spi_device *spi);
...
//para BMA020_WAKE_UP_PAUSE_20MS, BMA020_WAKE_UP_PAUSE_80MS, BMA020_WAKE_UP_PAUSE_320MS, BMA020_WAKE_UP_PAUSE_2560MS
...
//return BMA020_BW_25HZ, BMA020_BW_50HZ, BMA020_BW_100HZ, BMA020_BW_190HZ, BMA020_BW_375HZ, BMA020_BW_750HZ, BMA020_BW_1500HZ
#define		GET_BANDWIDTH		(0x09)

//para BMA020_BW_25HZ, BMA020_BW_50HZ, BMA020_BW_100HZ, BMA020_BW_190HZ, BMA020_BW_375HZ, BMA020_BW_750HZ, BMA020_BW_1500HZ
#define		SET_BANDWIDTH		(0x0A)


//BMA020_WAKE_UP_PAUSE_20MS, BMA020_WAKE_UP_PAUSE_80MS, BMA020_WAKE_UP_PAUSE_320MS, BMA020_WAKE_UP_PAUSE_2560MS
...
#define SET_HIGH_G_DUR						(0x42)

#define GET_HIGH_G_DUR						(0x43)
...
//BMA020_ANY_MOTION_DUR_1, BMA020_ANY_MOTION_DUR_3, BMA020_ANY_MOTION_DUR_5, BMA020_ANY_MOTION_DUR_7
...
//BMA020_INT_ALERT, BMA020_INT_ANY_MOTION, BMA020_INT_EN_ADV_INT, BMA020_INT_NEW_DATA, BMA020_INT_LATCH, BMA020_INT_HG, BMA020_INT_LG
//#define SET_INTERRUPT_MASK			(0x30)

//BMA020_INT_ALERT, BMA020_INT_ANY_MOTION, BMA020_INT_EN_ADV_INT, BMA020_INT_NEW_DATA, BMA020_INT_LATCH, BMA020_INT_HG, BMA020_INT_LG
//#define GET_INTERRUPT_MASK			(0x31)

//resets the BMA020 interrupt status, can be used to reset a latched interrupt
#define RESET_INT						(0x32)

// X,Y and Z-axis acceleration data readout, ref bma020acc_t structure for x,y,z data readout
#define READ_ACCEL_XYZ			(0x33)

//BMA020_INT_STATUS_HG, BMA020_INT_STATUS_LG, BMA020_INT_STATUS_HG_LATCHED, BMA020_INT_STATUS_LG_LATCHED, BMA020_INT_STATUS_ALERT, BMA020_INT_STATUS_ST_RESULT
...
#define WAIT_FOR_INT 			(0x3B)

#define SET_LOW_G_HYST			(0x44)
#define SET_HIGH_G_HYST			(0x45)

//general usage for simplicity
#define enable	(1)
#define disable	(0)
