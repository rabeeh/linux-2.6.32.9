...
__setup("ignore_extra_features", ignore_extra_features_setup);
...
static inline void write_extra_features(u32 u)
{
	if (ignore_extra_features)
		printk(KERN_WARNING "ignore_extra_features was set, skip "
		       "modifying the extra features register\n");
	else
		__asm__("mcr p15, 1, %0, c15, c1, 0" : : "r" (u));
}

static void disable_l2_prefetch(void)
{
	u32 u;
