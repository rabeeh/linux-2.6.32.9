...
cesa_ocf_init(void)
{
	/* we need to make sure crypto_init() called prior any crypto driver */
	if(!crypto_initted)
		crypto_init();
...
#endif
	/* register interrupt */
	if( request_irq( mv_crypto_irq_get(), cesa_interrupt_handler,
                             (IRQF_DISABLED) , "cesa", &cesa_ocf_id) < 0) {
            	printk("%s,%d: cannot assign irq %x\n", __FILE__, __LINE__, mv_crypto_irq_get());
		return EINVAL;
        }
...
cesa_ocf_exit(void)
{
	...
	free_irq(mv_crypto_irq_get(), NULL);
