				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	for (word=0 ; word < 4; word++) {
		for (i = 0; i < 4; i++){
		 ...
		 mv88fx_snd_writel(chip, 
				   MV_AUDIO_SPDIF_PLAY_CH_STATUS_RIGHT_REG(chip->port, word),
			chip->stream[PLAYBACK]->spdif_status[word]);
...
-----------------------------------------------------------------------------
*/
void cs42l51_vol_get(MV_U8 port, unsigned char *vol_list);
void cs42l51_vol_set(MV_U8 port, unsigned char *vol_list);
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	cs42l51_vol_get(chip->port, mv88fx_snd_vol);
...
static int mv88fx_snd_mixer_vol_put(struct snd_kcontrol *kcontrol, 
	...
	cs42l51_vol_set(chip->port, mv88fx_snd_vol);
...
mv88fx_snd_ctrl_new(struct snd_card *card)
{
	...
	if(chip->pdata->i2s_play && chip->pdata->spdif_play) {
		err = snd_ctl_add(card, snd_ctl_new1(
					  &mv88fx_snd_mixers[PLAYBACK_MIX_INDX],
					  chip));
		if (err < 0)
			return err;
	}
	...
	if(chip->pdata->i2s_rec && chip->pdata->spdif_rec) {
		err = snd_ctl_add(card, snd_ctl_new1(
					  &mv88fx_snd_mixers[CAPTURE_MIX_INDX],
					  chip));
		if (err < 0)
			return err;
	}
	...
	if(chip->pdata->i2s_rec || chip->pdata->spdif_rec) {
		err = snd_ctl_add(card, snd_ctl_new1(
					  &mv88fx_snd_mixers[CAPTURE_MONO_MIX_INDX],
					  chip));
		if (err < 0)
			return err;
	}
...
{
	int err;
	struct mv88fx_snd_chip *chip = substream->private_data;
...
//	substream->private_data = chip->stream_defaults[CAPTURE];
	substream->runtime->private_data = chip->stream_defaults[CAPTURE];
...
{
	struct mv88fx_snd_stream *audio_stream = substream->runtime->private_data;
	int err = 0; 
...
//		snd_pcm_substream_chip(substream);
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		/* make sure the dma in pause state*/
	     mv88fx_snd_bitset(chip, MV_AUDIO_RECORD_CTRL_REG(chip->port),
				  ARCR_RECORD_PAUSE_MASK);

		/* enable interrupt */
	     mv88fx_snd_bitset(chip, MV_AUDIO_INT_MASK_REG(chip->port), 
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		/* make sure the dma in pause state*/
	     mv88fx_snd_bitset(chip, MV_AUDIO_RECORD_CTRL_REG(chip->port),
                ...
                /* disable interrupt */
	     mv88fx_snd_bitreset(chip, MV_AUDIO_INT_MASK_REG(chip->port), 
				  AICR_RECORD_BYTES_INT);
...
//	substream->private_data = chip->stream_defaults[PLAYBACK];
	substream->runtime->private_data = chip->stream_defaults[PLAYBACK];
...
mv88fx_snd_playback_close(struct snd_pcm_substream * substream)
{
	...
	for (i=0; i<4 ; i++) {
		...
		chip->stream[PLAYBACK]->spdif_status[i] = 0;
		 ...
		 mv88fx_snd_writel(chip, 
				   MV_AUDIO_SPDIF_PLAY_CH_STATUS_LEFT_REG(chip->port, i),
			chip->stream_defaults[PLAYBACK]->spdif_status[i]);
		 mv88fx_snd_writel(chip, 
				   MV_AUDIO_SPDIF_PLAY_CH_STATUS_RIGHT_REG(chip->port, i),
			chip->stream_defaults[PLAYBACK]->spdif_status[i]);
...
mv88fx_snd_playback_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		/* make sure the dma in pause state*/
	     mv88fx_snd_bitset(chip, MV_AUDIO_PLAYBACK_CTRL_REG(chip->port),
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		/* check if busy twice*/
	     while (mv88fx_snd_readl(chip, MV_AUDIO_PLAYBACK_CTRL_REG(chip->port)) & 
		       APCR_PLAY_BUSY_MASK);
...
mv88fx_snd_playback_pointer(struct snd_pcm_substream * substream)
{
	...
	mv88fx_snd_debug("=>%s pointer %x\n",__FUNCTION__,mv88fx_snd_readl(chip, 
...
mv88fx_snd_pcm_new(struct snd_card *card)
{
	...
	for (i=0; i<4 ; i++) {
		...
		mv88fx_snd_writel(chip, 
			...
			chip->stream_defaults[PLAYBACK]->spdif_status[i]);
		mv88fx_snd_writel(chip, 
				  MV_AUDIO_SPDIF_PLAY_USR_BITS_LEFT_REG(chip->port, i),
			0);
		mv88fx_snd_writel(chip, 
				  MV_AUDIO_SPDIF_PLAY_USR_BITS_RIGHT_REG(chip->port, i),
			0);
...
mv88fx_snd_interrupt(int irq, void *dev_id)
{
	...
	mask = mv88fx_snd_readl(chip, MV_AUDIO_INT_MASK_REG(chip->port));
	status = mv88fx_snd_readl(chip, MV_AUDIO_INT_CAUSE_REG(chip->port)) & mask;
	...
	do {
		...
		mv88fx_snd_writel(chip, MV_AUDIO_INT_CAUSE_REG(chip->port), status);
...
mv88fx_snd_free(struct snd_card *card)
{
	...
	chip->base += AUDIO_REG_BASE(chip->port);
	if (chip->base)
		iounmap(chip->base);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
/* 0 is the playback dma */
#define WINDOW_CTRL(dma)		(0xA0C - ((dma) << 3))
#define WINDOW_BASE(dma)		(0xA08 - ((dma) << 3))

static int find_dram_cs(struct mbus_dram_target_info *dram,
			u32 base, u32 size)
{
	int i;
...
static int config_dma_window(struct mv88fx_snd_chip	*chip,
			     int dma,
			     struct mbus_dram_target_info *dram)
{
	 ...
	 */
	win = find_dram_cs(dram, 
			   (u32)chip->stream_defaults[dma]->addr,
			   (u32)MV88FX_SND_MAX_PERIODS * MV88FX_SND_MAX_PERIOD_BYTES);
	if( win < 0)
		return -1;
	...
	writel(cs->base, chip->base + AUDIO_REG_BASE(chip->port) + WINDOW_BASE(dma));
	return win;
}
static int mv_conf_mbus_windows(struct mv88fx_snd_chip	*chip,
				 struct mbus_dram_target_info *dram)
{
	/*
	 * we have one window for playback and one for recording
	 */
	if(config_dma_window(chip, PLAYBACK, dram) < 0)
		return -1;
	
	if(config_dma_window(chip, CAPTURE, dram) < 0)
		return -1;
	return 0;
}
#endif
static int mv88fx_snd_probe(struct platform_device *dev)
{
	...
	chip->pdata = (struct orion_i2s_platform_data*) dev->dev.platform_data;
        chip->port = dev->id;

	mv88fx_snd_debug("%s: port %d chip->pdata->dram %p\n",__FUNCTION__, 
			 chip->port, chip->pdata->dram);
	...
	if (chip->pdata->dram != NULL)
		if(mv_conf_mbus_windows(chip, chip->pdata->dram)){
			mv88fx_snd_debug(KERN_ERR "error: data buffer not in one memory window\n");
			err = -ENOMEM;
			goto error;
